        ...
        <header className="relative flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          >
             ...
             <div>
                ...
                <p className="text-2xl font-black text-slate-900">₹{todaysRevenue.toLocaleString('en-IN')}</p>
             </div>
          </motion.div>
        </header>

        {/* --- 2. STATS GRID WITH LAYERED ICONS --- */}
    ...
    >
      ...
      <div className="relative z-10 space-y-6">
        <div className="flex justify-between items-center">
          <div className={`p-4 rounded-2xl ${theme.bg} ${theme.text} transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 shadow-sm`}>
            <Icon size={28} strokeWidth={2} />
          </div>
          <div className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${
            isAlert ? 'bg-rose-500 text-white animate-pulse shadow-lg shadow-rose-200' : 'bg-slate-50 text-slate-500 font-bold border border-slate-100'
          }`}>
            {trend}
          </div>
        </div>
  ...
  <div className="flex flex-col lg:flex-row">
    <div className="lg:w-1/3 p-12 bg-rose-500 text-white flex flex-col justify-between relative overflow-hidden">
      ...
      <div className="space-y-6 relative z-10">
        <div className="h-16 w-16 rounded-[1.5rem] bg-white/20 backdrop-blur-xl border border-white/30 flex items-center justify-center shadow-2xl">
          <AlertTriangle size={32} />
        </div>
        <h3 className="text-4xl font-black tracking-tighter leading-[0.9]">Critical <br/> Inventory</h3>
        <p className="text-rose-100 font-medium text-lg leading-relaxed">{items.length} items require immediate restock to resume service.</p>
      </div>
      <Link to="/admin/products" className="mt-10 flex items-center gap-3 font-black uppercase text-[10px] tracking-[0.2em] bg-white text-rose-600 w-fit px-8 py-5 rounded-2xl hover:shadow-2xl transition-all active:scale-95 shadow-lg shadow-rose-700/20">
    ...
    <div className="flex-1 p-8 lg:p-12 space-y-4 max-h-[600px] overflow-y-auto bg-slate-50/30">
      {items.map((item, idx) => (
        <motion.div 
          ...
          key={item.id} 
          className="flex items-center justify-between p-5 bg-white rounded-[2rem] border border-slate-100 group/item hover:border-rose-200 hover:shadow-xl hover:shadow-rose-100/50 transition-all duration-300"
        >
          <div className="flex items-center gap-5">
            <div className="h-16 w-16 rounded-[1.2rem] overflow-hidden grayscale group-hover/item:grayscale-0 transition-all duration-700 shadow-inner">
              <img src={item.image || "https://via.placeholder.com/150"} alt="" className="h-full w-full object-cover" />
            </div>
            <div>
              <p className="font-black text-slate-800 text-lg tracking-tight group-hover/item:text-rose-600 transition-colors">{item.name}</p>
...
const EmptyStateSection = ({ count }) => (
  <div className="p-20 text-center space-y-8 bg-gradient-to-b from-white to-emerald-50/20">
    ...
    <div className="max-w-md mx-auto space-y-2">
      ...
      <p className="text-slate-500 font-medium text-lg leading-relaxed">All inventory is currently synchronized. No stock-outs detected across {count} items.</p>
    </div>
    <Link to="/admin/products" className="inline-flex items-center gap-3 px-10 py-5 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] hover:bg-indigo-600 hover:shadow-2xl hover:shadow-indigo-200 transition-all active:scale-95 shadow-xl shadow-slate-200">
